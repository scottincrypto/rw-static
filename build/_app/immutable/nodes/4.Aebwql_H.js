...
order by block_day desc limit 1`,ce=`select
  ...
  , (balance-token_balance)/balance as funds_allocated
from tokenlogic_data.treasury_returns
order by block_day desc limit 1`;a.current_day&&(a.current_day instanceof Error?W.initialError=a.current_day:W.initialData=a.current_day,a.current_day__DESCRIBE&&(W.knownColumns=a.current_day__DESCRIBE));let ae;const Q=K.createReactive({callback:P=>{l(0,ae=P)},execFn:u},{id:"current_day",...W});Q(ce,{noResolve:U}),globalThis[Symbol.for("current_day")]={get value(){return ae}};let X={initialData:void 0,initialError:void 0},ne=G`select distinct 
...
where 1=1
  and balance > 0
  and block_day = (select max(block_day) from tokenlogic_data.pool_returns)`;a.open_positions&&(a.open_positions instanceof Error?X.initialError=a.open_positions:X.initialData=a.open_positions,a.open_positions__DESCRIBE&&(X.knownColumns=a.open_positions__DESCRIBE));let pe;const Y=K.createReactive({callback:P=>{l(1,pe=P)},execFn:u},{id:"open_positions",...X});Y(re,{noResolve:ne}),globalThis[Symbol.for("open_positions")]={get value(){return pe}};let qe={initialData:void 0,initialError:void 0},ue=G`select 
  block_day
  , balance 
  , pnl
from tokenlogic_data.treasury_returns
order by block_day`,j=`select 
  ...
  , pnl
from tokenlogic_data.treasury_returns
order by block_day`;a.treasury_charts&&(a.treasury_charts instanceof Error?qe.initialError=a.treasury_charts:qe.initialData=a.treasury_charts,a.treasury_charts__DESCRIBE&&(qe.knownColumns=a.treasury_charts__DESCRIBE));let we;const J=K.createReactive({callback:P=>{l(2,we=P)},execFn:u},{id:"treasury_charts",...qe});J(j,{noResolve:ue}),globalThis[Symbol.for("treasury_charts")]={get value(){return we}};let Ie={initialData:void 0,initialError:void 0},fe=G`select 
  block_day
...
select 
  ...
  , apr 
from tokenlogic_data.treasury_returns
order by block_day, pool`;a.pool_apr&&(a.pool_apr instanceof Error?Me.initialError=a.pool_apr:Me.initialData=a.pool_apr,a.pool_apr__DESCRIBE&&(Me.knownColumns=a.pool_apr__DESCRIBE));let Te;const te=K.createReactive({callback:P=>{l(4,Te=P)},execFn:u},{id:"pool_apr",...Me});te(Z,{noResolve:me}),globalThis[Symbol.for("pool_apr")]={get value(){return Te}};let Ae={initialData:void 0,initialError:void 0},be=G`select 
  block_day
  , symbol
  , balance_usd
